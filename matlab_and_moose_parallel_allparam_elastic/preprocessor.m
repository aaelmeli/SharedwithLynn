...
spmd(length(inputs.freqs))
    ...
    for ipush=1:inputs.npush %start looping over all pushes, copy the input files
        runcmd = ['" cd  /home/aaelmeli/projects/first_test && cp ' sytheticinputfilename '_p' num2str(ipush) '".i" /home/aaelmeli/projects/first_test/' directoryname ' > NULL "'];
    status = system([bashcmd runcmd]);
    if (status ~= 0)
        error(['Run bash cmd error: ' num2str(status)]);
    end
    
        runcmd = ['" cd  /home/aaelmeli/projects/first_test && cp ' forwardinputfilename '_p' num2str(ipush) '".i" /home/aaelmeli/projects/first_test/' directoryname ' > NULL "'];
    status = system([bashcmd runcmd]);
    if (status ~= 0)
        error(['Run bash cmd error: ' num2str(status)]);
    end
