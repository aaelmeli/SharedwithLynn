...
bashcmd = 'bash -c '; % WSL
runcmd = ['" cp adjoint_real_force.csv /home/aaelmeli/projects/first_test/_adjoint_viscoelastic_waves"'];
...
bashcmd = 'bash -c '; % WSL
runcmd = ['" cp adjoint_imag_force.csv /home/aaelmeli/projects/first_test/_adjoint_viscoelastic_waves"'];
status = system([bashcmd runcmd]);
if (status ~= 0)
    error(['Run bash cmd error: ' num2str(status)]);
end
