...
bashcmd = 'bash -c '; % WSL
runcmd = ['"cd /home/aaelmeli/projects/first_test/_forward_viscoelastic_waves && cp  ' num2str(output.wavefieldoutputfilename_real) ' /mnt/e/3D_SWE_MOOSE/matlab_and_moose > NULL "'];
...
% ... - this for imaginary
bashcmd = 'bash -c '; % WSL
runcmd = ['"cd /home/aaelmeli/projects/first_test/_forward_viscoelastic_waves && cp  ' num2str(output.wavefieldoutputfilename_imag) ' /mnt/e/3D_SWE_MOOSE/matlab_and_moose > NULL "'];
status = system([bashcmd runcmd]); 
if (status ~= 0)
    error(['Run bash cmd error: ' num2str(status)]);
end
