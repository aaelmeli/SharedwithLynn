...
spmd(length(inputs.freqs))
    ...
    %make directories, 
        ...
        runcmd = ['" cd  /home/aaelmeli/projects/first_test/' directoryname ' && mkdir -p ' syntheticdirectory ' > NULL "'];
    status = system([bashcmd runcmd]);
    if (status ~= 0)
        error(['Run bash cmd error: ' num2str(status)]);
    end
    
        runcmd = ['" cd  /home/aaelmeli/projects/first_test/' directoryname ' && mkdir -p ' forwarddirectory ' > NULL "'];
    status = system([bashcmd runcmd]);
    if (status ~= 0)
        error(['Run bash cmd error: ' num2str(status)]);
    end
