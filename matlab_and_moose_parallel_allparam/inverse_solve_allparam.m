...
inputs.freqs=200:25:400; %the frequency range, in Hz.
...
%initial guess
params01=6000.*ones(inputs.mesh.nx*inputs.mesh.ny,1);
...
parameter_index_vector=[1:1:length(params)]';
...
param2=(80000).*ones(inputs.mesh.nx*inputs.mesh.ny,1);
param=[param1;param2];
...
% options = optimoptions(@fmincon,'Display','iter','SpecifyObjectiveGradient',true,'HessianFcn',[],'HessianMultiplyFcn',[],'HessianApproximation','lbfgs','SubproblemAlgorithm','cg', 'TolFun',1e-6,'MaxIterations',40);
tic;
