...
inputs.moosesyntheticinputfilename='synthetic_viscoacoustics_waves';%without the extension '.i' so that i can add which push number, for single push, we can add '.i' here
inputs.mooseforwardinputfilename='forward_viscoacoustics_waves'; %without the extension '.i' so that i can add which push number, for single push, we can add '.i' here
inputs.mooseadjointinputfilename='adjoint_viscoacoustics_waves.i';
inputs.moosegradientinputfilename='compute_gradient_viscoacoustics_waves.i';
...
inputs.moosesyntheticdirectory='_synthetic_viscoacoustics_waves';
inputs.mooseforwarddirectory='_forward_viscoacoustics_waves';
inputs.mooseadjointdirectory='_adjoint_viscoacoustics_waves';
inputs.moosegradientdirectory='grad_computation_viscoacoustics_waves';

inputs.materialfilename=["shearmodulus_dist.txt";"viscosity_dist.txt"];% %in case of inverting for E and eta
...
inputs.materialderivativefilename=["shearmodulus_dist_differentiated.txt";"viscosity_dist_differentiated.txt"]; %in case inverting for both E and eta
...
inputs.freqs=100:50:450; %the frequency range, in Hz.
...
nx=inputs.mesh.nx;nz=inputs.mesh.ny;
inputs.nparamx=nx;
inputs.nparamz=nz;
...
k=0;
element_ID=zeros(nx,nz);% matrix contains the element ids same as moose ids
for i=1:nx
    for j=1:nz
        k=k+1;
...
params01=log10(5e3).*ones(nparam,1);
params02=log10(1).*ones(nparam,1);
...
param1=log10(15e3).*ones(nparam,1);
param2=log10(3).*ones(nparam,1);
param=[param1;param2];
% param=[param1];
...
options = optimoptions(@fminunc,'Display','iter','Algorithm','quasi-newton','SpecifyObjectiveGradient',true, 'TolFun',1e-13,'MaxIterations',39);
